...
=item $el2 = $el->contextmenu

=item $el->contextmenu ($el2)

Return or set the element pointed by the C<contextmenu> attribute.
[HTML]
...
Return the L<Web::DOM::SettableTokenList> object for the C<dropzone>
...
=item $boolean = $el->inert

=item $el->inert ($boolean)

Return or set whether the C<inert> attribute is specified or not.
[HTML]
...
Return or set the C<itemid> attribute.  [HTML]

=item $tokens = $el->itemprop

=item $el->itemprop ($string)

Return the L<Web::DOM::SettableTokenList> object for the C<itemprop>
...
Return the L<Web::DOM::SettableTokenList> object for the C<itemref>
attribute, or set the C<itemref> attribute to the specified string
value.  [HTML]
...
Return the L<Web::DOM::SettableTokenList> object for the C<itemtype>
...
The L<Web::DOM::HTMLBaseFontElement> class is no longer available, as
it is now dropped from the HTML Standard.
...
CSSOM <http://dev.w3.org/csswg/cssom/#elementcssinlinestyle>.
