=head1 NAME

Web::DOM::HTMLParamElement - The DOM |HTMLParamElement| param

=head1 DESCRIPTION

The C<Web::DOM::HTMLParamElement> class implements the DOM
C<HTMLParamElement> interface.  The class is used by the HTML
C<param> element.

=head1 METHODS

In addition to the methods available for classes L<Web::DOM::Element>
and L<Web::DOM::HTMLElement>, the following methods are available:

=over 4

=item $string = $el->name

=item $el->name ($string)

Return or set the C<name> attribute.  [HTML]

=item $string = $el->value

=item $el->value ($string)

Return or set the C<value> attribute.  [HTML]

=back

Additionally, the obsolete C<type> and C<valuetype> methods are
available.  [HTML]

=head1 SPECIFICATION

=over 4

=item HTML

HTML Standard
<http://www.whatwg.org/specs/web-apps/current-work/#the-param-element>.

=back

=head1 SEE ALSO

L<Web::DOM::HTMLElement>.

=head1 AUTHOR

Wakaba <wakaba@suikawiki.org>.

=head1 LICENSE

Copyright 2013 Wakaba <wakaba@suikawiki.org>.

This library is free software; you can redistribute it and/or modify
it under the same terms as Perl itself.

=cut
