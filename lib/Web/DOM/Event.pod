...
Web::DOM::Event - The DOM |Event| object
...
=item $ev->init_event ($type, $bubbles, $cancelable)

Initialize the event.  [DOM]

Use the C<new> method instead.  This method does not have to be
invoked for event objects created by the C<new> method.
...
Following constants are used by the C<event_phase> method:
...
C<Event> interface, containing: L<Web::DOM::CustomEvent>.
...
DOM Living Standard <http://dom.spec.whatwg.org/#interface-event>.
