...
=item $et->add_event_listener ($type, $code, {capture => $boolean, passive => $boolean})
...
argument can specify whether the code listens the capturing phase
(true) or the bubbling phase (false, default) and whether the listner
is passive or not.  [DOM] [DOMPERL]
