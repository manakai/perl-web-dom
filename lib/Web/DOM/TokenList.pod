...
The C<DOMTokenList> object is used by the C<class_list> method of the
elements and the C<rel_list> method of the HTML hyperlink elements.
...
=item $boolean = $tokens->add ($token1, $token2, ...)
...
=item $tokens->replace ($token, $new_token)

Replace the token, if any, by a new token.  [DOM]
...
<http://wiki.suikawiki.org/n/manakai's%20DOM%20Perl%20Binding>.
